...
const Cart = () => {
  ...
  const handleCheckout = () => {
    alert(t('cart.checkoutAlert', { total }));
  };
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
          {items.length === 0 ? (
            ...
            <div className="max-w-2xl mx-auto text-center py-16">
              ...
              <h3 className="text-3xl font-bold text-gray-900 mb-4">{t('cart.empty')}</h3>
              <p className="text-gray-600 mb-8 text-lg">{t('cart.emptyDesc')}</p>
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex items-center space-x-4 text-sm">
                              <span className={`font-semibold px-2 py-1 rounded-full ${item.qualityScore >= 80 ? 'bg-green-100 text-green-800' : item.qualityScore >= 50 ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'}`}>
                                {t('cart.quality', { score: item.qualityScore })}
                              </span>
                ...
                <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
                  ...
                  <div className="space-y-4">
                    <label className="flex items-center p-4 rounded-2xl border-2 cursor-pointer transition-all duration-300 hover:border-green-300 hover:bg-green-50 group">
                      ...
                      <div className="ml-4 flex-1">
                        <div className="flex items-center justify-between">
                          <span className="text-lg font-semibold text-gray-900 group-hover:text-green-700 transition-colors duration-300">{t('cart.freePickup')}</span>
                          <span className="text-lg font-bold text-green-600">{t('cart.free')}</span>
                        </div>
                        <p className="text-gray-600 mt-1">{t('cart.freeDesc')}</p>
                      </div>
                    </label>
                    ...
                    <label className="flex items-center p-4 rounded-2xl border-2 cursor-pointer transition-all duration-300 hover:border-blue-300 hover:bg-blue-50 group">
                      ...
                      <div className="ml-4 flex-1">
                        <div className="flex items-center justify-between">
                          <span className="text-lg font-semibold text-gray-900 group-hover:text-blue-700 transition-colors duration-300">{t('cart.standard')}</span>
                          <span className="text-lg font-bold text-blue-600">200 DZD</span>
                        </div>
                        <p className="text-gray-600 mt-1">{t('cart.standardDesc')}</p>
                      </div>
                    </label>
                    ...
                    <label className="flex items-center p-4 rounded-2xl border-2 cursor-pointer transition-all duration-300 hover:border-purple-300 hover:bg-purple-50 group">
                      ...
                      <div className="ml-4 flex-1">
                        <div className="flex items-center justify-between">
                          <span className="text-lg font-semibold text-gray-900 group-hover:text-purple-700 transition-colors duration-300">{t('cart.express')}</span>
                          <span className="text-lg font-bold text-purple-600">500 DZD</span>
                        </div>
                        <p className="text-gray-600 mt-1">{t('cart.expressDesc')}</p>
                      </div>
                    </label>
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-br from-green-600 to-emerald-600 rounded-2xl shadow-xl p-8 text-white">
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center py-2">
                      <span className="text-green-100">{t('cart.subtotal', { count: items.length })}</span>
                      <span className="text-xl font-semibold">{subtotal} DZD</span>
                    </div>
                    <div className="flex justify-between items-center py-2">
                      <span className="text-green-100">{t('cart.shipping')}</span>
                  ...
                  <div className="mt-6 flex items-center justify-center space-x-2 text-green-200">
                    ...
                    <span className="text-sm">{t('cart.securePayment')}</span>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-2xl shadow-xl p-6 border border-gray-100">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="bg-gradient-to-br from-orange-50 to-orange-100 p-4 rounded-xl">
                      ...
                      <p className="text-sm text-orange-800">{t('cart.avgQuality')}</p>
                    </div>
                  </div>
                </div>
              </div>
