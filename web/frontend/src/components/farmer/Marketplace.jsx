    ...
    const fetchProfile = async () => {
      try {
        ...
        if (response.producer) {
          setCurrentUser({
            ...response.producer,
            name: response.producer.fullname,
            type: 'producer'
          });
        }
      } catch (err) {
                  ...
                  <div className="mb-8">
                    ...
                    <div className="space-y-3">
                      {[
                        ...
                        { value: 'Algiers', label: t('Algiers'), count: products.filter(p => p.region === 'Algiers').length },
                        { value: 'Oran', label: t('Oran'), count: products.filter(p => p.region === 'Oran').length },
                        { value: 'Constantine', label: t('Constantine'), count: products.filter(p => p.region === 'Constantine').length },
                        { value: 'Biskra', label: t('Biskra'), count: products.filter(p => p.region === 'Biskra').length },
                        { value: 'Sétif', label: t('Sétif'), count: products.filter(p => p.region === 'Sétif').length },
                        { value: 'Tlemcen', label: t('Tlemcen'), count: products.filter(p => p.region === 'Tlemcen').length }
                      ].map((option) => (
