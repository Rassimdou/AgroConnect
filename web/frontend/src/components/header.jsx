...
const Header = ({ isBuyerMode = false, isFarmerMode = false, isTransporterMode = false }) => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-linear-to-r from-white via-gray-50 to-white text-gray-700 shadow-2xl" style={{backgroundSize: '200% 200%', animation: 'flow 6s ease-in-out infinite'}}>
      ...
      <div className="container mx-auto px-4 sm:px-6 py-4 sm:py-6 flex justify-between items-center relative z-10">
        ...
        <div className="relative ml-4 language-selector">
          <button
            ...
            className="flex items-center space-x-2 px-3 py-2 bg-white rounded-lg border border-gray-300 hover:border-green-500 hover:bg-green-50 transition-all duration-200 shadow-md"
          >
            <span className="text-sm font-medium text-gray-700">
              {i18n.language === 'ar' ? 'العربية' : 'English'}
            </span>
            <svg
              className={`w-4 h-4 text-gray-500 transition-transform duration-200 ${isLangDropdownOpen ? 'rotate-180' : ''}`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          {isLangDropdownOpen && (
            <div className="absolute top-full right-0 mt-1 w-32 bg-white rounded-lg shadow-lg border border-gray-200 z-[100]">
              <button
