...
const OTPVerification = () => {
  ...
  const userFullnameFromQuery = params.get("user_fullname");
  const userRoleFromQuery = params.get("user_role");

  // Get user data from signup
  const userData = location.state?.userData;
  const mergedUserData = userData || {
    id: userIdFromQuery,
    fullname: userFullnameFromQuery,
    email: params.get("email"),
    role: userRoleFromQuery,
  };
  ...
  useEffect(() => {
    if (!mergedUserData?.id || !mergedUserData?.fullname || !(userRoleFromQuery || mergedUserData?.role)) {
      navigate('/signup');
    }
  }, [mergedUserData, userRoleFromQuery, navigate]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.post('/auth/verify/otp', {
        user_id: mergedUserData.id,
        user_fullname: mergedUserData.fullname,
        user_role: userRoleFromQuery || mergedUserData.role || 'user',
        otp: otpString
      });
      ...
      <div className={`max-w-md w-full space-y-8 transform transition-all duration-1000 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
        ...
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">
            We've sent a 4-digit code to <strong>{mergedUserData.email || 'your email'}</strong>
          </p>
