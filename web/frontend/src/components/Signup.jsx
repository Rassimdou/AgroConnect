...
const Signup = ({ userType = 'buyer' }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate(
        `/verify-otp?user_id=${encodeURIComponent(userId)}&user_fullname=${encodeURIComponent(formData.fullName)}&user_role=user`,
        { state: { userData: userState } }
      );
    } catch (error) {
      setErrors({ submit: error.message || t('Registration failed. Please try again.') });
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className={`max-w-md w-full space-y-8 transform transition-all duration-1000 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
        ...
        <div className="text-center">
          ...
          <p className="text-gray-600">
            {t('Create your account')} {userType}
          </p>
        </div>
        ...
        <div className="mt-8 text-center">
          <div className="bg-blue-50 rounded-lg p-4">
            <p className="text-sm text-blue-800">
              🚀 {t('Join thousands of farmers and businesses already using AgroConnect to transform their agricultural operations.')}
            </p>
          </div>
        </div>
      </div>
